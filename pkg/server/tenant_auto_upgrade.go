...
func (s *SQLServerWrapper) startAttemptUpgrade(ctx context.Context) error {
...
// func (s *SQLServerWrapper) upgradeStatus(
// 	ctx context.Context, clusterVersion clusterversion.ClusterVersion,
...
// 		if clusterVersion.String() == downgradeVersion {
...
func (s *SQLServerWrapper) upgradeStatus(
	...
	nodes, err := s.tenantStatus.Nodes(ctx, nil)
	if err != nil {
		return upgradeBlockedDueToError, err
	}
...
func (s *SQLServerWrapper) clusterVersion(ctx context.Context) (string, error) {
	row, err := s.sqlServer.internalExecutor.QueryRowEx(
